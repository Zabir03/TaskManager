...
async function init() {
  ...
  if (projectId) document.getElementById('filterProject').value = projectId;
  loadTasks();
}
    ...
    <div class="card" style="margin-bottom:0.8rem; padding:1rem">
      ...
      <div style="display:flex; gap:0.5rem; flex-wrap:wrap">
        <button onclick="openStatusModal(${t.id}, '${t.title}', '${t.status}')"
          style="width:auto; padding:0.3rem 0.8rem; font-size:0.8rem; background:#e0e7ff;
          color:#3730a3; border:none; border-radius:6px; cursor:pointer; margin-top:0">
          🔄 Status
        </button>
        ${user.role === 'admin' ? `
          <button onclick="deleteTask(${t.id})"
            style="width:auto; padding:0.3rem 0.8rem; font-size:0.8rem; background:#fee2e2;
            color:#dc2626; border:none; border-radius:6px; cursor:pointer; margin-top:0">
            🗑 Delete
          </button>` : ''}
      </div>
    </div>
...
async function createTask() {
  ...
  if (!title || !project_id) {
    msg.textContent = '⚠️ Title and Project are required!';
  ...
  if (res.ok) {
    ...
    setTimeout(() => { closeCreateModal(); loadTasks(); }, 1000);
  } else {
...
async function updateStatus() {
  ...
  if (res.ok) { closeStatusModal(); loadTasks(); }
}
...
function openCreateModal() { document.getElementById('createTaskModal').classList.add('active'); }
